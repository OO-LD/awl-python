...
SCHEMA_URL = "https://w3id.org/battery/schema/ChargeParam.schema.json?v=1.2.0"


def test_a_constructor_may_name_its_schema():
    """`$schema` carries what a bare class name cannot: the version, the field
    ranges a form needs, and the declared types an exporter materializes.

    Only a constructor carries it. Its sibling keys are the author's field
    names, so only there is a plain word unsafe, and `$` cannot appear in a
    Python identifier.
    """
    from awl.compact import encode

    types = {"ChargeParam": {**TYPES["ChargeParam"], "schema": SCHEMA_URL}}
    doc = collapse(_folded(), types=types, resolved=RESOLVED, embed_context=False)
    node = _find(encode(doc), lambda item: isinstance(item.get("@type"), list))
    assert node["$schema"] == SCHEMA_URL
    assert node["@type"][0] == "ChargeParam", "the bare term stays, so nothing must resolve"


def test_the_schema_is_additive_and_never_invented():
    """A URL guessed from an import path would validate and resolve to nothing."""
    doc = collapse(_folded(), types=TYPES, resolved=RESOLVED, embed_context=False)
    node = _find(doc, lambda item: isinstance(item.get("@type"), list))
    assert "$schema" not in node


def test_naming_a_schema_does_not_stop_the_document_regenerating_code():
    from awl.compact import decode, encode
...
def test_a_field_named_type_would_not_collide_with_the_schema_key():
    """Why `$schema` rather than a word: a constructor's siblings are the
    author's field names, and nothing stops a class declaring `schema`.
    """
    types = {
        "ChargeParam": {
            **TYPES["ChargeParam"],
            "schema": SCHEMA_URL,
            "fields": [{"name": "target_voltage"}, {"name": "c_rate"}, {"name": "schema"}],
        }
    }
    doc = collapse(
        _folded("ChargeParam(target_voltage=4.2, c_rate=0.23, schema='local')\n"),
        types=types,
        resolved=RESOLVED,
        embed_context=False,
    )
    node = _find(doc, lambda item: isinstance(item.get("@type"), list))
    assert node["$schema"] == SCHEMA_URL, "the OO-LD schema"
    assert node["schema"] == "local", "the author's field, untouched"
