...
__all__ = ["collapse", "expand"]

#: Keys of a collapsed node that are structure rather than field data.
_RESERVED = frozenset({"@context", "@type", "@", "_callee", "order"})


def collapse(doc: Any, *, types: dict[str, Any], resolved: dict[str, str]) -> Any:
    ...
    if isinstance(doc, list):
        return [collapse(item, types=types, resolved=resolved) for item in doc]
    ...
    if doc.get("_type") == "Call":
        ...
        if info is not None:
            return _typed_node(doc, info, types=types, resolved=resolved)

    return {key: collapse(value, types=types, resolved=resolved) for key, value in doc.items()}
...
def _typed_node(call: dict[str, Any], info: dict[str, Any], **kw: Any) -> dict[str, Any]:
    ...
    declared = info.get("declaredTypes") or []
    node: dict[str, Any] = {
        "@context": {"@vocab": info["identity"]["iri"] + "#"},
        "@type": declared[0] if declared else info["identity"]["iri"],
        # Reversibility: @type is an IRI, not the name written in the source,
        # so without _callee the node cannot be unparsed back.
        "_callee": call.get("func", {}).get("id"),
        "@": [
            call.get("lineno"),
            call.get("col_offset"),
            call.get("end_lineno"),
...
def expand(node: Any) -> Any:
    ...
    if not isinstance(node, dict):
        return node
    if "@type" in node and "_callee" not in node:
        raise ValueError("typed node has no _callee, so it cannot be expanded")
    if "_callee" not in node:
    ...
    return {
        "_type": "Call",
        "func": {"_type": "Name", "id": node["_callee"], "ctx": {"_type": "Load"}, **position},
        "args": [],
        "keywords": keywords,
        **position,
    }
